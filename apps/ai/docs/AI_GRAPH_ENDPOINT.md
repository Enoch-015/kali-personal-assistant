...
9. **Plugin Dispatch (LLM-supervised)**: Executes the planned action with the selected plugin using tool-style plans generated by the reasoning agent
...
The graph's email channel is now backed by the Resend API. During dispatch the reasoning agent produces a tool-style plan that can adjust recipients, payload fields, and metadata before Resend executes the send.

Configure Resend via environment variables in `apps/ai/.env`:

- `RESEND_API_KEY`: API key provided by Resend (required for live delivery).
- `RESEND_FROM_ADDRESS`: Optional default `from` address.
- `RESEND_DEFAULT_RECIPIENT`: Optional fallback recipient when the request omits recipients.
- `RESEND_DELIVER`: Set to `true` to enable live sends; defaults to dry-run mode for local testing.

When `RESEND_DELIVER` is left unset or `false`, the plugin runs in dry-run mode and reports the intended recipients without calling the external API, making it safe for tests and development.
